...
async function render() {
  ...
  if (monthLabel) monthLabel.textContent = `${y}??${m}??;
...
function updateSelectedInfo() {
  ...
  selectedInfo.textContent = selectedDate ? `?†ÌÉù: ${fmt(selectedDate)}` : "?†ÌÉù ?†Ïßú ?ÜÏùå";
}
...
qSheet.innerHTML = `
  <div class="head">
    ...
    <div class="sheet-actions">
      <button class="btn" id="qClose">?´Í∏∞</button>
      <button class="btn primary" id="qSave">?Ä??/button>
    </div>
  </div>
  <div class="body">
    <div class="field">
      <div class="label">?¨ÏßÑ</div>
      <input id="qFiles" type="file" accept="image/*" multiple />
      <div class="quick-preview" id="qPreview"></div>
    </div>
    <div class="field">
      <label class="label" for="qNote">Î©îÎ™®</label>
      <textarea id="qNote" placeholder="Í∞ÑÎã®??Í∏∞Î°ù?¥Ïöî."></textarea>
    </div>
  </div>
`;
...
qFiles.addEventListener("change", () => {
  ...
  [...(qFiles.files || [])].forEach((file) => {
    if (!ACCEPTED_IMAGE_TYPES.test(file.type || "")) {
      alert(`"${file.name}"?Ä ÏßÄ?êÌïòÏßÄ ?äÎäî ?¥Î?ÏßÄ ?ïÏãù?¥Ïóê??`);
      return;
    }
    if (file.size > MAX_UPLOAD_BYTES) {
      alert(`"${file.name}" ?åÏùº?Ä ${Math.round(MAX_UPLOAD_BYTES / (1024 * 1024))}MBÎ•?Ï¥àÍ≥º?àÏñ¥??`);
      return;
    }
...
qSave.addEventListener("click", async () => {
  ...
  if (!files.length && !text) {
    alert("?¥Ïö©?¥ÎÇò ?¥Î?ÏßÄÎ•??ÖÎ†•??Ï£ºÏÑ∏??");
    return;
  }
  const originalLabel = qSave.textContent;
  qSave.disabled = true;
  qSave.textContent = "?ÖÎ°ú??Ï§?..";
  try {
    ...
    console.error(e);
    alert("?ÖÎ°ú??Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏñ¥??");
  } finally {
    qSave.disabled = false;
    qSave.textContent = originalLabel || "?Ä??;
  }
});
...
async function onClickDate(d) {
  ...
  if (dateStr === fmt(today)) {
    if (confirm("?§Îäò Í∏∞Î°ù???®Í∏∏ÍπåÏöî?")) {
      openQuickDiary(dateStr);
      return;
    }
  }

  const title = prompt("?ºÏ†ï ?úÎ™©???ÖÎ†•?òÏÑ∏??);
  if (!title) return;
  try {
    const token = await getCsrfToken();
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      alert(data?.error || "?ºÏ†ï ?Ä?•Ïóê ?§Ìå®?àÏñ¥??");
      return;
    }
    ...
    console.error(e);
    alert("?ºÏ†ï ?Ä??Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏñ¥??");
  }
}
...
async function openViewer(dateStr) {
  ...
  if (metaEl) {
    const meta = [];
    if (photos.length) meta.push(`${photos.length}???¨ÏßÑ`);
    if (schedItems.length) meta.push(`${schedItems.length}Í∞??ºÏ†ï`);
  ...
  if (photosEl) {
    ...
    if (photos.length) {
      photos.forEach((p, idx) => {
        const img = document.createElement("img");
        img.src = p.image_url;
        img.alt = `${dateStr} ?¨ÏßÑ ${idx + 1}`;
        photosEl.appendChild(img);
      });
    } else {
      const ph = document.createElement("div");
      ph.className = "ph";
      ph.textContent = "?¨ÏßÑ???ÜÏäµ?àÎã§";
  ...
  if (schedEl) {
    ...
    if (schedItems.length) {
      ...
      schedItems.forEach((s) => {
        ...
        title.textContent = s.title || "(?úÎ™© ?ÜÏùå)";
